...
public class DownloadController {
	...
	private ModelAndView index(@RequestParam(value = "s", defaultValue = "10") Integer size, @RequestParam(value = "p", defaultValue = "0") Integer number, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
		...
		org.springframework.data.domain.Page<Download> pageOfEntities;
		if (queryString == null) {
			pageOfEntities = downloadRepository.findAll(pageRequest);
		} else {
			pageOfEntities = downloadRepository.findAll(tw.org.itri.mii.specification.DownloadSpecs.titleSpec(queryString), pageRequest);
		}
		...
		for (Download entity : pageOfEntities.getContent()) {
			Element rowElement = Utils.createElementWithAttribute("row", contentElement, "id", entity.getId().toString());//列(帶主鍵)
			Utils.createElementWithTextContent("filename", rowElement, entity.getFilename());//原始檔名
		...
		ModelAndView modelAndView = new ModelAndView("download/default");
	...
	private ModelAndView create(HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
		...
		Document document = Utils.newDocument();
		Element documentElement = Utils.createElementWithAttribute("document", document, "contextPath", request.getContextPath());
		documentElement.setAttribute("title", "後臺 &#187; 下載專區 &#187; 新增");

		services.whereTheHellAmI(document, request, session);

		Element formElement = Utils.createElementWithAttribute("form", documentElement, "action", request.getRequestURI());
		formElement.setAttribute("legend", "新增下載檔案");
		downloadService.load(new Download(), formElement);

		ModelAndView modelAndView = new ModelAndView("download/form");
		modelAndView.getModelMap().addAttribute(new DOMSource(document));
		return modelAndView;
	}
	...
	private ModelAndView create(@RequestParam(required = false, value = "content") MultipartFile multipartFile, @DateTimeFormat(pattern = "yyyy-MM-dd") @RequestParam(defaultValue = "") Date since, @DateTimeFormat(pattern = "yyyy-MM-dd") @RequestParam(defaultValue = "") Date until, @RequestParam boolean censored, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
		...
		Element formElement = Utils.createElementWithAttribute("form", documentElement, "action", request.getRequestURI());
		formElement.setAttribute("legend", "新增下載檔案");
		String errorMessage = downloadService.save(new Download(), multipartFile, since, until, censored, formElement);
		...
		return new ModelAndView("redirect:/XiaZaiZhuanQu/");
	}
	...
	private ModelAndView read(@PathVariable Short id, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
		...
		Download entity = downloadRepository.findOne(id);
		if (entity == null) {
		...
		Document document = Utils.newDocument();
		Element documentElement = Utils.createElementWithAttribute("document", document, "contextPath", request.getContextPath());
		documentElement.setAttribute("title", "後臺 &#187; 下載專區 &#187; 修改");

		services.whereTheHellAmI(document, request, session);

		Element formElement = Utils.createElementWithAttribute("form", documentElement, "action", request.getRequestURI());
		formElement.setAttribute("legend", "修改下載檔案");
		downloadService.load(entity, formElement);
	...
	private ModelAndView update(@PathVariable Short id, @RequestParam(required = false, value = "content") MultipartFile multipartFile, @DateTimeFormat(pattern = "yyyy-MM-dd") @RequestParam(defaultValue = "") Date since, @DateTimeFormat(pattern = "yyyy-MM-dd") @RequestParam(defaultValue = "") Date until, @RequestParam boolean censored, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
		Short myId = services.whoTheHeckAmI(session);
		if (myId == null) {
			return new ModelAndView("redirect:/DengRu.asp");//未登入則導向到登入頁面
		}
		...
		Element formElement = Utils.createElementWithAttribute("form", documentElement, "action", request.getRequestURI());
		formElement.setAttribute("legend", "修改下載檔案");
		String errorMessage = downloadService.save(entity, multipartFile, since, until, censored, formElement);
		if (errorMessage != null) {
			formElement.setAttribute("errorMessage", errorMessage);
			ModelAndView modelAndView = new ModelAndView("download/form");
			modelAndView.getModelMap().addAttribute(new DOMSource(document));
