			...
			<FIELDSET>
				...
				<TABLE class="form">
					...
					<xsl:if test="id">
						<TR>
							<TH>
								<LABEL for="{generate-id(filename)}">原始檔名</LABEL>
							</TH>
							<TD>
					...
					<TR>
						<TH>
							...
							<LABEL for="{generate-id(content)}">檔案</LABEL>
						</TH>
						<TD>
							<INPUT id="{generate-id(content)}" name="content" type="file" value="{content}"/>
						</TD>
						<TD>
							<xsl:if test="not(id)">必選；請侷限於各類文件檔格式。</xsl:if>
						</TD>
					</TR>
					<xsl:if test="id">
						<TR>
							<TH>
								<LABEL for="{generate-id(contentType)}">檔案類型</LABEL>
							</TH>
							<TD>
